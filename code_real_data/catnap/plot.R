library(ggpubr)
library(tidyverse)
plot_one_folder <- function(folder) {
  ...
  ggsave(gg1, filename = paste(folder, 'panel.pdf', sep = ""), width = 5, height = 4)
  ...
  ggsave(
    ...
    filename = paste(folder, '0.05.pdf', sep = ""), width = 7, height = 4
  )

}

load("./result_npboot/npbootstrap.rda")
plot_one_folder("./result_npboot/")
load("./result_permutey/permute_y.rda")
plot_one_folder("./result_permutey/")
